...
const dbFolder = userDir$1 + (NODE_ENV$3 === "development" ? "/.easychatdev/" : "/.easychat/");
...
const getDomain = () => {
  return NODE_ENV$2 !== "development" ? store$1.getData("prodDomain") : store$1.getData("devDomain");
};
...
const initWs = (config, _sender) => {
  wsUrl = `${NODE_ENV$1 !== "development" ? store$1.getData("prodWsDomain") : store$1.getData("devWsDomain")}?token=${config.token}`;
  sender = _sender;
  needReconnect = true;
  maxReConnectTimes = 5;
  createWs();
