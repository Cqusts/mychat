...
一个仿微信的全栈即时通讯应用，在完整的 IM 系统之上构建了 AI Agent 能力：内置的单聊助手能调用业务工具查数据，配置的多个群助手能被拉进群、被 @ 触发，并在群里互相接力讨论。
...
| 类型 | 标识 | 触发方式 | 工具调用 | 多轮记忆 |
|------|------|----------|----------|----------|
| 内置单聊助手 | `Constants.ROBOT_UID` | 私聊直接对话 | ✅ | ✅ Redis |
| 群助手 | `ai.agents[n]` 配置 | 群里被 @ 昵称 | ❌ 见"已知限制" | ❌ 用群聊记录作上下文 |
...
**单聊助手**：注册后通讯录里自带一个智能助手，直接聊即可。试试这些能触发工具调用的问题：
...
```properties
ai.agents[3].id=Uagentops
ai.agents[3].name=运维小O
ai.agents[3].signature=只关心线上会不会炸
ai.agents[3].prompt=你是一名运维工程师，习惯从部署复杂度、监控告警和故障恢复的角度思考问题。
```

> `id` 必须以 `U` 开头且不超过 12 个字符——`user_info.user_id` 字段长度限制。重启后助手账号会自动创建。
...
- **群助手不响应私聊**。单聊的 AI 分支只对内置的 `ROBOT_UID` 生效，把群助手加为好友后私聊它不会有回复。
